...
if(temp != NULL)
{
 ...
 {
	 if((strcmp(temp->word , src)) < 0) //a negative value if the first differing character in lhs is less than the corresponding character in rhs.
	 {
		 search(temp->right , src);

	 }
	 else if((strcmp(temp->word , src)) > 0)//a positive value if the first differing character in lhs is greater than the corresponding character in rhs.
	 {
